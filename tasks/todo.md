...
- **Auto-submit, and the three decisions it forced:**
  ...
  2. **`readOnly` + `aria-disabled` while signing in,** not `disabled`, which would drop the field out of the tab order and throw away focus mid-flow. The restart button uses a real `disabled`.
...
- **Verified:** `pnpm lint`, `pnpm typecheck`, 45 unit tests, `pnpm build` and **10 Playwright tests** all pass. The browser tests assert the three widths, the resting *and* focused field borders, that the code step has no "Sign in" button, that filling six digits submits on its own, and that the digits stay focused after a wrong code.
